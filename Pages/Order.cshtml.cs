    ...
    public class OrderModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var email = Request.Form["OrderEmail"];
                var entry = db.Orders.Add(new Order());
                entry.CurrentValues.SetValues(email);
                //entry.CurrentValues.SetValues(OrderShipping);
                //entry.CurrentValues.SetValues(OrderQuantity);
